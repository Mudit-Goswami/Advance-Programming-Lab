...
int main() {
	...
	int info=0;
    FILE *f = fopen("02_w.txt","r");
    FILE *f2 = fopen("02_c.txt","r");
    ...
    const int limit = 6404180; // knapsack weight limit
    const int pop = 1000000; // chromosome population size
...
//					crossoverarit(ch[0],ch[1],ch[i],dimw);
		...
		if(p%5==0) {
            ...
            if(best == 6404180) goto end; // psst...don't tell anyone
		}
		best = avg = 0;
	}
